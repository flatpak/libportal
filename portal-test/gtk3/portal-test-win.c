  ...
  while (g_variant_iter_next (iter, "(u@a{sv})", &id, &props))
    {
      ...
      g_strv_builder_take (builder, g_strdup_printf ("Stream %d: %dx%d @ %d,%d", id, w, h, x, y));
  ...
  if (device_types)
    {
      ...
      g_autofree char* devices_s = NULL;

      if (device_types & XDP_DEVICE_KEYBOARD)
        g_strv_builder_add (b, "keyboard");
      if (device_types & XDP_DEVICE_POINTER)
        g_strv_builder_add (b, "pointer");
      if (device_types & XDP_DEVICE_TOUCHSCREEN)
        g_strv_builder_add (b, "touchscreen");

      devices = g_strv_builder_end (b);
      devices_s = g_strjoinv (", ", devices);

      g_strv_builder_take (builder, g_strdup_printf("Devices: %s", devices_s));
    }

  g_strv_builder_take (builder,
                       g_strdup_printf ("Clipboard: %s",
                                        xdp_session_is_clipboard_enabled (session) ? "yes" : "no"));
  strv = g_strv_builder_end (builder);
  s = g_strjoinv (", ", strv);

  if (xdp_session_get_session_type (session) == XDP_SESSION_SCREENCAST)
    gtk_label_set_label (GTK_LABEL (win->screencast_label), s);
  else if (xdp_session_get_session_type (session) == XDP_SESSION_REMOTE_DESKTOP)
    gtk_label_set_label (GTK_LABEL (win->remotedesktop_label), s);
}
