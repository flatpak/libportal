...
struct _PortalTestWin
{
  ...
  GtkWidget *image;
  GtkCheckButton *interactive;

  GtkCheckButton *inhibit_idle;
  GtkCheckButton *inhibit_logout;
  GtkCheckButton *inhibit_suspend;
  GtkCheckButton *inhibit_switch;
  int inhibit_cookie;
  XdpInhibitFlags inhibit_flags;

  GtkWidget *username;
  GtkWidget *realname;
  GtkWidget *avatar;
  GtkWidget *save_how;
  GtkCheckButton *open_local_ask;
  GtkCheckButton *open_local_dir;
...
portal_test_win_init (PortalTestWin *win)
{
  ...
  if (proxies == NULL)
    {
      g_message ("Failed to lookup proxies: %s", error->message);
      proxy = g_strdup ("");
    }
  else
            ...
            gpointer data)
{
  ...
  open_dir = gtk_check_button_get_active (win->open_local_dir);
...
open_local (GtkWidget *button, PortalTestWin *win)
{
  ...
  if (gtk_check_button_get_active (win->open_local_ask))
...
static const char *filechooser_options[] = {
  "current",
  "iso8859-15",
  "utf-16",
  NULL
};
static const char *filechooser_labels[] = {
...
static void
on_response (GtkNativeDialog *dialog, int res, PortalTestWin *win)
{
  ...
  if (res == GTK_RESPONSE_ACCEPT)
    {
      ...
      if (strcmp (method, "atomically") == 0)
        result = write_file_atomically (filename, &error);
      else if (strcmp (method, "posix") == 0)
        result = write_file_posix (filename, &error);
      else if (strcmp (method, "gio") == 0)
        result = write_file_gio (filename, &error);
      else
  ...
  label = "";
  for (i = 0; filechooser_options[i]; i++)
    {
      if (g_strcmp0 (encoding, filechooser_options[i]) == 0)
        label = filechooser_labels[i];
    }
...
save_dialog (GtkWidget *button, PortalTestWin *win)
{
  ...
  parent = GTK_WINDOW (gtk_widget_get_root (button));
  ...
  g_signal_connect (dialog, "response", G_CALLBACK (on_response), win);
                 ...
                 PortalTestWin *win)
{
  ...
  if (gtk_check_button_get_active (win->interactive))
...
{
  XdpInhibitFlags flags = 0;

  if (gtk_check_button_get_active (win->inhibit_logout))
    flags |= XDP_INHIBIT_FLAG_LOGOUT;
  if (gtk_check_button_get_active (win->inhibit_switch))
    flags |= XDP_INHIBIT_FLAG_USER_SWITCH;
  if (gtk_check_button_get_active (win->inhibit_suspend))
    flags |= XDP_INHIBIT_FLAG_SUSPEND;
  if (gtk_check_button_get_active (win->inhibit_idle))
    flags |= XDP_INHIBIT_FLAG_IDLE;
            ...
            gpointer    data)
{
  ...
  g_message ("message %d", GST_MESSAGE_TYPE (message));
  ...
  if (GST_MESSAGE_TYPE (message) == GST_MESSAGE_ERROR)
  ...
  else if (GST_MESSAGE_TYPE (message) == GST_MESSAGE_STATE_CHANGED)
    {
      ...
      if (change == GST_STATE_CHANGE_SUCCESS)
        g_message ("Pipeline state now %d", state);
      else if (change == GST_STATE_CHANGE_ASYNC)
        g_message ("Pipeline state now %d, pending %d", state, pending);
      else
        g_message ("Pipeline state change failed, now %d", state);
...
play_sound (gdouble frequency)
{
  ...
  g_message ("Set pipeline state to playing");
...
portal_test_win_class_init (PortalTestWinClass *class)
{
  ...
  gtk_widget_class_set_template_from_resource (widget_class,
                                               "/org/gnome/portal-test/gtk4/portal-test-win.ui");
